...
@if(Session::get('status') && Session::get('status') == 2000)
  ...
  <form action="{{route('login')}}" method="POST" class="php-email-form">
    ...
    <div class="form-group">
      <input placeholder="Email address" type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email') }}" required />
      @error('email')
    ...
    <div class="form-group">
      <input placeholder="Password" type="password" class="form-control @error('password') is-invalid @enderror" name="password" required/>
      @error('password')
          <span class="invalid-feedback" role="alert">
              <strong>{{ $message }}</strong>
          </span>
